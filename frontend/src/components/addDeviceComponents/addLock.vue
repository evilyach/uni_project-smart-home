<template>
  <div>
    <div class="text-h4 q-mb-md">Умный замок</div>
    ...
    <q-checkbox
      ...
      filled
      right-label
      label="Уведомлять об инцидентах безопасности?"
      style="width: 99%"
    />
      ...
      pushData() {
        ...
        axios
          ...
          .then(() => {
            ...
            addLogRecord({
              log_level: "info",
              message: `Добавлено устройство "Умный замок" с названием "${this.name}"`
            });
          })
          .catch(e => {
            this.$q.notify({
            ...
            addLogRecord({
              log_level: "error",
              message: `Неудачная попытка добавить устройство "Умный замок" с названием "${this.name}"`
            });
          });
      }
    };
