<template>
  <div>
    <div class="text-h4 q-mb-md">Интеллекутальный чайник</div>
    ...
    <q-input
      ...
      filled
      label="Температура"
      hint="Введите температуру работы чайника"
      style="width: 99%"
    />
      ...
      pushData() {
        ...
        axios
          ...
          .then(() => {
            ...
            addLogRecord({
              log_level: "info",
              message: `Добавлено устройство "Интеллектуальный чайник" с названием "${this.name}"`
            });
          })
          .catch(e => {
            this.$q.notify({
            ...
            addLogRecord({
              log_level: "error",
              message: `Неудачная попытка добавить устройство "Интеллектуальный чайник" с названием "${this.name}"`
            });
          });
      }
    };
