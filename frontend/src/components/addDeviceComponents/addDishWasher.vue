<template>
  <div>
    <div class="text-h4 q-mb-md">Посудомоечная машина</div>
    ...
    <q-checkbox
      ...
      filled
      right-label
      label="Уведомлять об готовности?"
      style="width: 99%"
    />
    ...
    <q-input
      ...
      filled
      label="Температура"
      hint="Введите температуру работы посудомоечной машины"
      style="width: 99%"
    />
    ...
    <q-input
      ...
      filled
      label="Влажность"
      hint="Введите влажность внутри посудомоечной машины"
      style="width: 99%"
    />
    ...
    <q-input
      ...
      filled
      label="Время"
      hint="Введите время работы (в секундах)"
      style="width: 99%"
    />
  ...
  data() {
    return {
      ...
      pushData() {
        ...
        if (isNaN(this.speed) === true || this.speed < 10800) {
          this.$q.notify({
            message: "Введено неправильное время работы!",
            color: "warning"
          });
          return;
        }
        ...
        axios
          ...
          .then(() => {
            ...
            addLogRecord({
              log_level: "info",
              message: `Добавлено устройство "Посудомоечная машина" с названием "${this.name}"`
            });
          })
          .catch(e => {
            this.$q.notify({
            ...
            addLogRecord({
              log_level: "error",
              message: `Неудачная попытка добавить устройство "Посудомоечная машина" с названием "${this.name}"`
            });
          });
      }
    };
