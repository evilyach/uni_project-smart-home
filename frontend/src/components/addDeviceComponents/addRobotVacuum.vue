<template>
  <div>
    <div class="text-h4 q-mb-md">Робот-пылесос</div>
    ...
    <q-input
      ...
      hint="Введите максимальную скорость движения"
      style="width: 99%"
    />
    ...
    <q-input
      v-model="max_value"
      filled
      label="Максимальное расстояние от базовой станции"
      hint="Введите максимальное расстояние работы робота-пылесоса от базовой станции"
      style="width: 99%"
    />
  ...
  data() {
    return {
      ...
      pushData() {
        ...
        if (isNaN(this.speed) === true || this.speed < 0 || this.speed > 30) {
          this.$q.notify({
            message: "Введена некорректная скорость!",
            color: "warning"
          });
          return;
        }
        ...
        if (
          isNaN(this.max_value) === true ||
          this.max_value < 0 ||
          this.max_value > 30
        ) {
          this.$q.notify({
            message: "Введена некорректное расстояние от базовой станции!",
            color: "warning"
          });
          return;
        }
        ...
        axios
          ...
          .then(() => {
            ...
            addLogRecord({
              log_level: "info",
              message: `Добавлено устройство "Робот-пылесос" с названием "${this.name}"`
            });
          })
          .catch(e => {
            this.$q.notify({
            ...
            addLogRecord({
              log_level: "error",
              message: `Неудачная попытка добавить устройство "Робот-пылесос" с названием "${this.name}"`
            });
          });
      }
    };
